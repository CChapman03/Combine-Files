...
def print_stats():
    ...
    for col in output_collections.children:
        ...
        if col.children is not None:
            for subcol in col.children:
                ...
                for ob in subcol.objects:
                    ...
                    ob_str = "\t\t\tObject: %s, Type: %s, Polys : %s, Materials: %s\n" % (str(ob.data.name), str(objType), str(polys[0]), str(mat_names[0]))
                    print(ob_str)
        ...
        for ob in col.objects:
            ...
            print("\t\tObject: %s, Type: %s, Polys : %s, Materials: %s\n" % (str(ob.data.name), str(objType), str(polys[0]), str(mat_names[0])))    
    ...
    file_path = "%s/%s" % (out_directory, output_filename)
